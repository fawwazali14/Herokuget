...
def fetch_data():
    try:
        ...
        if table == "Job_Listings":
            ...
            print("solo")
            print(x)
            job_fetch_query = f"SELECT job_ID, job_title, time_posted, Job_Listings.description, pay_per_hr, duration, cyclic, Job_Listings.ID,is_open,name FROM Job_Listings Join Users on Job_Listings.ID = Users.ID WHERE Job_Listings.ID !='{x}' ;"
